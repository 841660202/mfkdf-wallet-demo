  ...
  go() {
    ...
    (async () => {
      ...
      if (user.length === 0) {
        ...
        this.setState({
          loading: false,
          error: 'Please enter at least two factors.',
        });
      } else {
        try {
          ...
          const factors = {};
          if (pass.length > 0) {
            factors.password = mfkdf.derive.factors.password(pass);
          }
          if (hmac.length > 0) {
            factors.hmacsha1 = mfkdf.derive.factors.hmacsha1(hmac);
          }
          ...
          this.setState({
            loading: false,
            error: "Couldn't find that wallet.",
          });
        }
      }
    })();
